# Finotary — Complete Project Knowledge Base
...
**Runtime:** Dev Client APK via EAS Build (dev) → Production Build (production). *Note: Expo Go cannot run this project due to SDK 55 vs Expo Go's SDK 52 ceiling.*
**Framework:** React Native via Expo SDK ~55
**Platform tested:** Android
**Developer OS:** Windows 11 | PowerShell

**Note on Package Name / Bundle Identifier:** The package name/bundle identifier change (from `com.abhiramkasturi19.Finova` to `com.abhiramkasturi19.Finotary` in `app.json`) only applies if no build has been uploaded to the Play Console yet under the old name.

### Current App Version
**1.0.1 — Version Reset + Pro Persistence Fix + Demo Theme Fix + Demo Exit Symmetry + Keyboard Coverage Fix**

### Delivered Files (v1.0.1)
...
### Previously Delivered Files (v1.0.1, still current)
...
### 10.2 LoginScreen *(updated v1.0.1)*
JSON backup → validate → importData() → hasOnboarded:'true' → reset to Main.
Auto-detects FINOTARY_ENC: or FINOTARY_ENC2: prefix → DecryptImportModal.

**DecryptImportModal keyboard fix (v1.0.1):**
...
**PinSetupModal keyboard fix (v1.0.1):**
...
- **Try Demo / Exit Demo** — same row, conditional on `isDemoMode` (v1.0.1):
...
- Download Data — Pro-gated (JSON backup). Automatically injects an `exportMeta` object with `username`, `exportedAt` timestamp, and `appVersion`.
- CSV Export — Pro feature. Uses a **CSV Export Filter Modal** (`CsvExportModal`).
  - **Selectors:** Two independent selectors: Time Period (`This Month`, `This Year`, `Choose Year`, `Choose Month`, `All Time`) and Wallet (`All Wallets`, or individual wallets).
  - **Live Preview:** Displays the transaction count for the current selection. The "Download CSV" button is disabled if the count is `0`.
  - **Embedded Meta-Data:** Injects a `#` comment block atop the CSV containing `Username`, `Exported At`, and `App Version`. Parsers natively skip these comments.
  - **Filename Auto-Generation:** Generated filenames incorporate a sanitized username and local date, e.g., `Finotary_John_Doe_2026-07-06.csv`.
  - **Category Labels:** CSV categories are resolved via `getCat()` from `src/data/categories.js` instead of printing raw category IDs.
  - **Helper Functions:** Added `getYearsWithData`, `filterByPeriod`, `filterByWallet`, `buildCsvFilename`, and `generateCsvString`.
- Passcode Export — Pro feature. Uses the same dynamic filename convention and `exportMeta` tagging.
...
### 10.7 AddTransactionScreen *(v1.0.1)*
...
### 10.13 Demo Mode *(v1.0.1 — theme + exit fixes)*
...
**Theme in demo (v1.0.1 fix):** Demo loads with the user's real `darkMode` preference, not `DEMO_DATA.settings.darkMode`. `LOAD_DEMO` reducer explicitly copies `state.settings.darkMode` into the demo state. `toggleDarkMode()` / `UPDATE_SETTINGS` with only `darkMode` key is unblocked in demo — all other settings writes remain blocked.
...
Exit restores _realStateSnapshot (real theme, wallets, settings, transactions).
...
### Persist useEffect — CRITICAL RULE (v1.0.1)
```js
useEffect(() => {
...
### LOAD_DEMO reducer — CRITICAL RULE (v1.0.1)
```js
case 'LOAD_DEMO': {
  const snapshot = {
    transactions:     state.transactions,
    settings:         { ...state.settings },   // includes darkMode, everything
...
### EXIT_DEMO reducer — CRITICAL RULE (v1.0.1)
```js
case 'EXIT_DEMO': {
...
### UPDATE_SETTINGS in demo — darkMode exception (v1.0.1)
```js
...
| `toggleDarkMode()` | Shorthand | Allowed in demo (v1.0.1) |
...
### isPro & App Lock persistence *(clarified v1.0.1)*
...
### Screen → Preset mapping *(v1.0.1 — current)*
...
XOR encryption — FINOTARY_ENC2: format: salt + hash-derived key + positional shift. Both v1 (FINOTARY_ENC:) and v2 supported for decryption. Upload flow auto-detects prefix.
...
| HIGH Weak XOR Encryption | Static passcodes | FINOTARY_ENC2 salted-hash positional shift |
...
| **Android themed/monochrome icons rendering incorrectly (Android 13+)** | `android-icon-foreground.png` and `android-icon-monochrome.png` have solid white background instead of alpha transparency | *Fix in progress:* Re-export both as transparent PNGs, same size/logo scale (logo occupies ~60% of canvas, within required 66% safe zone) |
...
| 136–170 | v2.9.0 through v1.0.1 (Pro System, App Lock, Wallets, Search, Demo Mode) | — |
...
| 191 | Version bumped to 1.0.1 | KB |

---

## 20. PRO SYSTEM — ARCHITECTURE & MONETIZATION

The Pro system has been fully removed. The app is 100% free and all features are available to all users unconditionally.

---

## 21. HOW TO RUN & TROUBLESHOOTING

### Standard Run & Dev Workflow
- **Expo Go Incompatibility:** Expo Go can never run this project due to SDK 55 vs Expo Go's SDK 52 ceiling. This is a permanent version ceiling incompatibility, not a bug.
- **Correct Dev Workflow:**
  1. Run `eas build --profile development --platform android` to get a development client APK.
  2. Install the resulting APK once on the test device.
  3. Start the Metro server using `npx expo start --dev-client --clear`.
  4. Open the app using the installed dev-client app. Never use Expo Go.
  *Note:* Always use the `--clear` flag after dropping in new files, as Metro cache can cause EventEmitter crashes otherwise.
...
### Demo Mode security
Never written to AsyncStorage. All writes are pure no-op except darkMode toggle (v1.0.1). No toast or error on blocked actions.
...
8. **`updateSettings`** for profile fields.
...
13. **JSON backup contains everything** including wallets and activeWalletId.
14. **AddTransaction + AppGuide + Wallets all use `panDownManual`.**
15. **Wallets uses `panDownManual`** — changed from slideRight in v1.0.1. Do NOT revert.
...
21. **Modal sheet: `#2C3020`. AppLock bg: `#1A1D1A`.**
22. **TermsModal "I Understand" does NOT auto-tick.**
23. **`executeClear` uses `AsyncStorage.setItem` NOT `AsyncStorage.clear()`.**
24. **`executeClear` preserves `customCategories`, `appLockEnabled`, `appLockPin`, `wallets`, `activeWalletId`.**
...
34. **AddTransaction and AppGuide spring stiffness is `240`, damping `26`.**
35. **WalletsScreen uses `panDownManual`** — NO internal Animated.View.
36. **`addCustomCategory` returns `'ok'` or `'demo_mode'`.**
37. **`addWallet` returns `'ok'` or `'demo_mode'`.**
...
50. **Demo mode writes are ALL silently blocked** — except darkMode toggle (v1.0.1).
...
60. **Persist useEffect MUST have `if (state.isDemoMode) return` guard** — demo data must never overwrite real state in AsyncStorage. (v1.0.1)
61. **LOAD_DEMO MUST preserve `state.settings.darkMode`** — demo renders in user's real theme. (v1.0.1)
62. **EXIT_DEMO MUST return full `_realStateSnapshot`** — never initialState. (v1.0.1)
63. **UPDATE_SETTINGS in demo: unblock darkMode-only writes** — all other settings remain blocked. (v1.0.1)
64. **Settings Try Demo / Exit Demo is a single conditional row** — same location, same section, switches based on isDemoMode. (v1.0.1)
65. **`softwareKeyboardLayoutMode:'pan'` MUST be in `app.json` android section** — base fix for all keyboard coverage issues. (v1.0.1)
66. **PinSetupModal: vertically centred, paddingBottom:300** — keyboard must not cover PIN dots. (v1.0.1)
67. **DecryptImportModal in LoginScreen: KAV + justifyContent flex-end + marginBottom:40** — keyboard must not cover password input. (v1.0.1)
...
| AppGuide footnote update | v1.0.1.x | Update version reference |

---

*Last updated: May 12, 2026*
*Version: 1.0.1 — Pro Persistence Fix + Demo Theme Fix + Demo Exit Symmetry + Keyboard Coverage Fix*
